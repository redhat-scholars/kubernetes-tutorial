...
Startup probes fixes this problem, as once the startup probe has succeeded, the rest of the probes takes over 
...
The startup probe waits for one minute (`5 * 12`) to startup the application.

The current application returns a `503` error in `/alive` endpoint so it will never succeed to boot up and `restartPolicy` is applied.
...
Wait for 60 seconds until you see that the pod is restarted.

And you will see it restarted:
...
To make the pod succeed, `exec` into it and reborn it:
...
[source,bash,subs="+macros,+attributes"]
----
kubectl delete deplpoyment myboot
----