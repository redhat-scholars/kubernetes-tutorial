...
Now that we're there, let's watch the `/mnt/data` directory that the pod has mounted as `/tmp/demo`

[.console-input]
[source,bash,subs="+macros,+attributes"]
----
watch -n1 -- eval "ls -al /mnt/data && echo $(cat /mnt/data/greeting.txt)"
----
