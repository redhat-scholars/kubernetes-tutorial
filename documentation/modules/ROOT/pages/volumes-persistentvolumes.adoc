...
Let's deploy a service that exposes two endpoints, one to write content to a file and another one to retrieve the content from that file.  Use kbd:[CMD+p] to quickly open `myboot-pod-volume.yml`
...
Let's access into the container:
...
You'll notice that the directory content is empty.
...
Use kbd:[CMD+p] to open `myboot-pods-volume.yml` quickly: 
...
Let's access into the *first* container in one terminal
...
And then in another terminal `exec` into the other container in the pod to run the `cat` command
...
And then from inside the `myboot-demo-2` container in the pod, run the following command (similar to the one we ran in the other `myboot-demo-1` container): 
...
Now exit out of the `exec` to the pod
...
include::partial$terminal-cleanup.adoc[]

=== HostPath

A `hostPath` volume mounts a file or directory from the node's filesystem into the Pod.

Use kbd:[CMD+p] to quickly open `myboot-pod-volume-hostpath.yml`
...
Meanwhile in the other terminal you should at the same time see the watch output change
...
Now from the main terminal you should be able to delete the pods
...
Let's watch the path on the node that the _PV_ maps to
...
While the watch is going in Terminal 2 run the following commands in Terminal 1
...
----
NAME                 STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS   AGE
myboot-volumeclaim   Bound    pvc-09be221f-7036-4626-b2ee-831fdc0af558   10Mi       RWO            standard       6s
----
--
OpenShift::
+
