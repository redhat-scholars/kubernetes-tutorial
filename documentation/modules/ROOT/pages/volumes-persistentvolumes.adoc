...
Let's use `hostPath` strategy but not configuring it directly as volume, but using persistence volume and persistence volume claim.  Use kbd:[CMD+p] to open `demo-persistent-volume-hostpath.yaml` quickly:
...
In the following example, we are requesting for *10Mi* space.  Use kbd:[CMD+p] to open `myboot-persistent-volume-claim.yaml` quickly:
...
The big difference is that now in the pod you are just defining in the `volumes` section, not the volume configuration directly, but the _persistent volume claim_ to use.  Use kbd:[CMD+p] to open `myboot-pod-volume-pvc.yaml` quickly:
